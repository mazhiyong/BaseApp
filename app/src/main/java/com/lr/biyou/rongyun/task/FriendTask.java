...
import com.lr.biyou.rongyun.model.GetContactInfoResult;
import com.lr.biyou.rongyun.model.PhoneContactInfo;
...
import com.lr.biyou.rongyun.net.service.FriendService;
...
public class FriendTask {
     ...
     * @return
     */
    public LiveData<Resource<FriendShipInfo>> getFriendInfo(String token,String userId) {
        Log.i("show","获取好友信息");
        return new NetworkBoundResource<FriendShipInfo, Map<String,Object>>() {
            @Override
            protected void saveCallResult(@NonNull Map<String,Object> item) {
               /* UserDao userDao = dbManager.getUserDao();
                FriendDao friendDao = dbManager.getFriendDao();
             ...
             /*   switch (item.get("code") + "") {
                    case "0": //请求成功
                        FriendInfo friendInfo = new FriendInfo();
                        if(!UtilTools.empty(item.get("data")+"")){
                           Map<String,Object> map= (Map<String, Object>) item.get("data");
                           friendInfo.setId(friendShipInfo.getUser().getId());
                           friendInfo.setMessage(friendShipInfo.getMessage());
                    }
