...
import com.lr.biyou.ui.temporary.adapter.BorrowSelectAdapter;
 ...
 * 我要借款 （应收账款池  信用融资）
 */
public class BorrowMoneySelectActivity extends BasicActivity implements RequestView, ReLoadingData {
    ...
    BorrowSelectAdapter mBorrowSelectAdapter;
    ...
    public int getContentView() {
        return R.layout.activity_brorow_money_select;
    }
        ...
        switch (mType){
            case MethodUrl.jiekuanSxList:
                if(mBorrowSelectAdapter!=null){
                    if(mBorrowSelectAdapter.getDataList().size()<=0){
    ...
    private void responseData() {
        if (mBorrowSelectAdapter == null) {
            mBorrowSelectAdapter = new BorrowSelectAdapter(this);
            mBorrowSelectAdapter.addAll(mDataList);

            AnimationAdapter adapter1 = new ScaleInAnimationAdapter(mBorrowSelectAdapter);
            ...
            mLRecyclerViewAdapter = new LRecyclerViewAdapter(mBorrowSelectAdapter);
